...
public class RateLimit {
	...
	public boolean isNotLimited() throws InterruptedException {
		if (curCount >= REQUEST_COUNT) {
			synchronized (syncObject) {
				logger.log(Level.WARNING, String.format("%s has reached its rate limit (%d/%d). Reset in %ds", NAME,
						curCount, REQUEST_COUNT, getResetTime()));
				syncObject.wait();
			}
		}
	...
	private long getResetTime() {
		return ((lastResetTime + TIME_INTERVAL) - System.currentTimeMillis()) / 1000;
	}
}
